...
int main(int argc, char **argv)
{
    ...
    if(rank == 0){
        ...
        globalBuf = (Pixel *)data;
        Pixel **matriz = initMainMatriz(pixelCast, height, width);
    } 
       
    // Broadcast de dimensiones 
    int fullData [3] = {width, height, channels};
    MPI_Bcast(fullData, 3, MPI_INT, 0, MPI_COMM_WORLD); 
    width = fullData[0];
    height = fullData[1];
    channel = fullData[2];

    // Distribucion de bloques
    int *sendCounts = (int *)malloc(size * sizeof(int)); // filas propias por proceso
    int *sendDispls = (int *)malloc(size * sizeof(int)); // display en filas en imagen
    ...
    for(int i = 0; i < size; i++){
        int rowsForProcess = baseRows + (i < extraRows ? 1: 0);
        sendCounts[i] = rowsForProcess * width * sizeof(Pixel);
        sendDispls[i] = offset * width * sizeof(Pixel);
        offset += rowsForProc;
    }

    int localRows = sendCounts[rank]; // filas propias de este proceso

    int localHeightWithHalo = localRow + 2 * radio;
    
    //Buffer local con HALO (integra el padding) cada fila aloja fila + halos
    Pixel *localPool = (Pixel*)calloc(localHeightWithHalo * width, sizeof(Pixel)); 
    Pixel **localMatriz = (Pixel **)malloc(localHeightWithHalo * sizeof(Pixel *));
    for(int i = 0; i < localHeightWithHalo; i++){
        localMatriz[i] = localPool[i*width];
    }

    if(rank == 0){
        MPI_Scatterv(pixelCast, send)
    }



    Pixel *localBuf = (Pixel *)malloc(haloRows * width * sizeof(Pixel));
