...
int blurMatriz3x3(Pixel **input, Pixel **output, int width, int height, int radio){
    ...
    for(int i = 1; i < heightAux; i++){
        ...
        // inferior
        r = 0; g = 0; b = 0;
        for(int f = heightAux - 1; f <= heightAux; f++){
            for(int c = i - 1; c <= i + 1; c++){
                Pixel neighbours = input[f][c];
                r += neighbours.r;
                g += neighbours.g;
        ...
        output[i][heightAux].r = (unsigned char)(r/6);
        output[i][heightAux].g = (unsigned char)(g/6);
        output[i][heightAux].b = (unsigned char)(b/6); 
    }
    
    //bordes laterales
    for(int i = 1; i < widthAux; i++){
        int r = 0, g = 0, b= 0;
        ...
        r = 0; g = 0; b = 0;
        for(int f = i - 1; f <= i + 1; f++){
            for(int c = widthAux - 1; c <= widthAux ; c++){
                Pixel neighbours = input[f][c];
                r += neighbours.r;
                g += neighbours.g;
                b += neighbours.b;
        ...
        output[i][widthAux].r = (unsigned char)(r/6);
        output[i][widthAux].g = (unsigned char)(g/6);
        output[i][widthAux].b = (unsigned char)(b/6); 
    }

    // for para el centro
    for(int i = 1; i < widthAux; i ++){
        for(int j = 1; j < heightAux; j++){
    ...
    // esq sup der 
    output[0][widthAux].r = (unsigned char)(input[0][0].r + input[0][1].r + input[1][0].r + input[1][1].r) /4;
    output[0][widthAux].g = (unsigned char)(input[0][0].g + input[0][1].g + input[1][0].g + input[1][1].g) /4;
    output[0][widthAux].b = (unsigned char)(input[0][0].b + input[0][1].b + input[1][0].b + input[1][1].b) /4;
