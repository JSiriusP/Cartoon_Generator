...
paradigma de control para tareas posterizado y borroneado
paradigma de datos para datos de entrada 

Las tareas de posterizado y borroneado puede realizarse de forma paralela, dado que no tienen dependencia entre ellas. 
De esta forma, se decidió paralelizarlas asignandole cierta cantidad de recursos a cada sección.

Para la seccion de borroneado, al contar con dos tareas dependientes entre sí se opta por implementar un pipeline de datos. 
El mismo consta de ejecutar primero la funcion de blur, seguida de la función de highlight (adicionando la operación de umbralado). 

## Algoritmo secuencial

Tanto en este, como en los demas algoritmos se utilizo la tecnica de Padding, esta consta de agregar a la matriz inicial, un borde auxiliar con pixeles neutros. De esta forma, al momento de hacer el procesamiento de blur y highlighted, se descarta la necesidad de corroborar si la celda vecinas validas.

## Algoritmo con Memoria compartida

` comentario: agregar args que se pasan` 

Para el diseño del algoritmo, se optó por aplicar descomposición de datos de entrada para cada tarea dentro del proceso. 

Al momento de paralelizar las tareas de borroneado y psoterizado se aplicaron las directivas #pragma omp parallel sections num_threads(2) para identificar las secciones a paralelizar (las dos tareas) y luego la directiva #pragma omp section a cada una de ellas, para realizar en forma paralela las distintas secciones.

Tareas:
    * suma, posterizado
Se hizo uso de la directiva #pragma omp parallel for schedule (static)
PSe divide el espacio total de forma estática, dado que el costo de computacional de cada iteración es constante, a diferencia de otras directivas como dynamic. (ampliar si corresponde)


    * Highlight, BLur
Se aplicó la directiva #pragma omp parallel for schedule (static) para inicialización de datos.
Luego para el propio desarrollo de las tareas se emplea la directiva #pragma omp parallel for collapse(2) schedule(guided). En este caso se utiliza guided ya que el trabajo en cada iteración puede ser variable o impredecible, ademas, guided es más eficiente que dynamic en la administración

## con Memoria Distribuida
` comentario: agregar args que se pasan` 
Para el diseño del algoritmo, como previamente se ha aplicado, se decide que el proceso 0 
sea el orquestador del resto de los procesos. 
Los procesos workers se dividen a la mitad de acuerdo a las secciones previamente definidas (posterizado y borroneado). 
A medida que se testeó el tiempo de ejecución de cada algoritmo, nos percatamos de que al momento
de paralizar la tarea de suma de las matrices resultantes de las operaciones borroneado y posterizado, el tiempo incrementaba considerablemente respecto del hecho de no paralelizar tal operación. Debido a tales resultados, se decidió no aplicar paralelismo a esa función. 
La suma final f
...
# Resultados de ejecutar en local
